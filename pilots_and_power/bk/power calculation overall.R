...
#effects.
#Note: this only works if the absolute value of the addon is below one.
get_latent_addon <- function(addon, discrete_var, selection_var=NULL){
  
  #if a selection variable was given, select only obsverations
  #that scored a 1 on it
  if(is.null(selection_var)){
    filter <- rep(TRUE, times=length(discrete_var))
  } else {
    filter <- selection_var==1
  }
...
#function to sample from the original dataset, add treatment effects to achieve the desired ATE,
#and estimate the ATE and its significance for one outcome by regressing wave 2 outcome on
#treatment and wave 1

simulate_main <- function(d, add_to_latent, outcome, outcome_name_latent_w2, 
                          outcome_name_addon_w2, n_wave2, prop_treat){
  
  ##Step 1: randomly assign to treatment
  
  d$Treated_sim <- rbinom(n=nrow(d), size=1, prob=prop_treat)
  
  ##Step 2: add scenario's treatment effect
  
  #add scenario's treatment effect to the latent outcome for simulated treated
  #participants
  outcome_latent_w2 <-  ifelse(d$Treated_sim,
  ...
  ##Step 3: sample with replacement to get n observations
  
  n_treated <- n_wave2 * prop_treat
  n_control <- n_wave2 - n_treated
  sample_treated <- sample(which(d$Treated_sim==1), size=n_treated, replace=T)
  sample_control <- sample(which(d$Treated_sim==0), size=n_control, replace=T)
...
power_byoutc <- lapply(outcomes, function(outcome){
  ...
  message("calculating power for outcome: ", outcome)
    ...
    power_byaddon <- lapply(ATEs[[outcome]], function(ATE){
      ...
      #get quantity to be added to latent w2 outcome (adjusting for floor/ceiling effects)
      add_to_latent <- get_latent_addon(addon=ATE,
                                        discrete_var = d[[outcome_name_nulsim_w2]])
      ...
      power_out <- pbreplicate(250, simulate_main(d, add_to_latent,
                                                 outcome = outcome,
