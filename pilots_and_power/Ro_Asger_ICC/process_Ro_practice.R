...
dF <- read.delim("Article Characteristics Coding Final (Responses) - Short column names.tsv", sep="\t",
...
names(nature_binary) <- paste0("nature,",
                               c( "Blockade", "Interrupting", "Attached", "AlteratVandal"))
                               #rename columns
...
dF_kept <- dF[c("id",
                ...
                "nature,Interrupting",
                "portrayal")]
write.csv(dF_kept, "Ro_practice_clean.csv", row.names=F)
